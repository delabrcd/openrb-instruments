...
public:
    ...
    static void init() {
        USB_SPI.begin();  // The SPI library with transaction will take care of setting up the pins
                          // - settings is set in beginTransaction()
        SPI_SS::SetDirWrite();
        SPI_SS::Set();
    }
...
#warning "You need to initialize the SPI interface manually when using the STM32F4 platform"
    ...
    static void init() {
        ...
        USB_SPI.setClockSpeed(12000000);  // The MAX3421E can handle up to 26MHz, but in practice
                                          // this was the maximum that I could reliably use
#else
        USB_SPI.setClockDivider(
            SPI_CLOCK_DIV2);  // This will set the SPI frequency to 8MHz - it could be higher, but
                              // it is not supported in the old API
#endif
#elif !defined(RBL_NRF51822) && !defined(NRF52_SERIES)
        USB_SPI.setClockDivider(
            4);  // Set speed to 84MHz/4=21MHz - the MAX3421E can handle up to 26MHz
#endif
    }
#else
    static void init() {
        ...
        SPI_MISO::SetDirRead();
        SPI_SS::SetDirWrite();
        /* mode 00 (CPOL=0, CPHA=0) master, fclk/2. Mode 11 (CPOL=11, CPHA=11) is also supported by
         * MAX3421E */
        SPCR = 0x50;
        SPSR = 0x01;  // 0x01
                      /**/
...
typedef SPi<MAKE_PIN(PIN_SPI_SCK), MAKE_PIN(PIN_SPI_MOSI), MAKE_PIN(PIN_SPI_MISO),
            MAKE_PIN(PIN_SPI_SS)>
    spi;
#undef MAKE_PIN
#elif defined(__AVR_ATmega1280__) || (__AVR_ATmega2560__) || defined(__AVR_ATmega32U4__) || \
    defined(__AVR_AT90USB646__) || defined(__AVR_AT90USB1286__)
typedef SPi<Pb1, Pb2, Pb3, Pb0> spi;
#elif defined(__AVR_ATmega168__) || defined(__AVR_ATmega328P__)
typedef SPi<Pb5, Pb3, Pb4, Pb2> spi;
#elif defined(__AVR_ATmega644__) || defined(__AVR_ATmega644P__) || defined(__AVR_ATmega1284__) || \
...
typedef SPi<Pb7, Pb5, Pb6, Pb4> spi;
    ...
    defined(__ARDUINO_ARC__) || defined(__ARDUINO_X86__) || defined(__MIPSEL__) ||    \
    defined(STM32F4)
...
MAX3421e<SPI_SS, INTR>::MAX3421e() {
    ...
    // For Mega ADK, which has a Max3421e on-board, set MAX_RESET to output mode, and then set it to
    // HIGH
    P55::SetDirWrite();
    P55::Set();
#endif
...
void MAX3421e<SPI_SS, INTR>::regWr(uint8_t reg, uint8_t data) {
    ...
    USB_SPI.beginTransaction(SPISettings(
        26000000, MSBFIRST,
        SPI_MODE0));  // The MAX3421E can handle up to 26MHz, use MSB First and SPI mode 0
#endif
    SPI_SS::Clear();
