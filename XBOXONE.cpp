...
void XBOXONE::EndpointXtract(uint8_t conf, uint8_t iface __attribute__((unused)),
    ...
    if ((pep->bmAttributes & bmUSB_TRANSFER_TYPE) ==
        USB_TRANSFER_TYPE_INTERRUPT) {  // Interrupt endpoint
        index = (pep->bEndpointAddress & 0x80) == 0x80
                    ? XBOX_ONE_INPUT_PIPE
                    : XBOX_ONE_OUTPUT_PIPE;  // Set the endpoint index
    } else
        return;
...
#endif
    if (pollInterval < pep->bInterval)  // Set the polling interval as the largest polling interval
...
void XBOXONE::readReport() {
    ...
    if (readBuf[0] != 0x20) {  // Check if it's the correct report, otherwise return - the
                               // controller also sends different status reports
#ifdef EXTRADEBUG
        Notify(PSTR("\r\nXbox Poll: "), 0x80);
    ...
    uint16_t xbox =
        ButtonState & pgm_read_word(&XBOX_BUTTONS[ButtonIndex(
                          XBOX)]);  // Since the XBOX button is separate, save it and add it back in
    // xbox button from before, dpad, abxy, start/back, sync, stick click, shoulder buttons
    ButtonState = xbox | (((uint16_t)readBuf[5] & 0xF) << 8) | (readBuf[4] & 0xF0) |
...
uint16_t XBOXONE::getButtonPress(ButtonEnum b) {
    ...
    const int8_t index = getButtonIndexXbox(b);
    if (index < 0)
        return 0;
...
void XBOXONE::setRumbleOn(uint8_t leftTrigger, uint8_t rightTrigger, uint8_t leftMotor,
                          uint8_t rightMotor) {
    uint8_t writeBuf[13];
