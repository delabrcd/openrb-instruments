...
#define XBOX_WIRELESS_MAX_ENDPOINTS 9

/**
 * This class implements support for a Xbox Wireless receiver.
 *
 * Up to four controllers can connect to one receiver, if more is needed one can use a second
 * receiver via the USBHub class.
 */
...
public:
    ...
    /**
     * getButtonPress(uint8_t controller, ButtonEnum b) will return true as long as the button is
     * held down.
     *
     * While getButtonClick(uint8_t controller, ButtonEnum b) will only return it once.
     *
     * So you instance if you need to increase a variable once you would use getButtonClick(uint8_t
     * controller, ButtonEnum b), but if you need to drive a robot forward you would use
     * getButtonPress(uint8_t controller, ButtonEnum b).
     * @param  b          ::ButtonEnum to read.
     * @param  controller The controller to read from. Default to 0.
     * @return            getButtonClick(uint8_t controller, ButtonEnum b) will return a bool, while
     * getButtonPress(uint8_t controller, ButtonEnum b) will return a byte if reading ::L2 or ::R2.
     */
...
protected:
    ...
    EpInfo epInfo[XBOX_WIRELESS_MAX_ENDPOINTS];
...
private:
    ...
    };                          // General purpose buffer for input data
    ...
    void printReport(uint8_t controller,
                     uint8_t nBytes);  // print incoming date - Uncomment for debugging
