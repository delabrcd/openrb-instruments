/* Copyright (C) 2012 Kristian Lauszus, TKJ Electronics. All rights reserved.
 ...
 getBatteryLevel and checkStatus functions made by timstamp.co.uk found using BusHound from
 Perisoft.net
 */
...
XBOXRECV::XBOXRECV(USB *p)
    ...
    for (uint8_t i = 0; i < XBOX_WIRELESS_MAX_ENDPOINTS; i++) {
        epInfo[i].epAddr      = 0;
...
uint8_t XBOXRECV::ConfigureDevice(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    p->epinfo = epInfo;     // Temporary assign new pointer to epInfo to p->epinfo in order to avoid
                            // toggle inconsistence
    p->lowspeed = lowspeed;

    rcode = pUsb->getDevDescr(0, 0, constBufSize,
    ...
    if ((VID != XBOX_VID && VID != MADCATZ_VID && VID != JOYTECH_VID) ||
        (PID != XBOX_WIRELESS_RECEIVER_PID_1 && PID != XBOX_WIRELESS_RECEIVER_PID_2 &&
         PID != XBOX_WIRELESS_RECEIVER_THIRD_PARTY_PID)) {  // Check if it's a Xbox receiver using
    ...
    bAddress = addrPool.AllocAddress(parent, false,
                                     port);  // Allocate new address according to device class
...
uint8_t XBOXRECV::Init(uint8_t parent __attribute__((unused)), uint8_t port __attribute__((unused)),
                       bool lowspeed) {
    uint8_t rcode;
    ...
    rcode = pUsb->setEpInfoEntry(bAddress, 1,
                                 epInfo);  // Assign epInfo to epinfo pointer - only EP0 is known
    if (rcode)
        goto FailSetDevTblEntry;

    /* The application will work in reduced host mode, so we can save program and data
       memory space. After verifying the VID we will use known values for the
       configuration values for device, interface, endpoints and HID for the XBOX360 Wireless
       receiver */
    ...
    epInfo[XBOX_INPUT_PIPE_1].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_OUTPUT_PIPE_1].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_INPUT_PIPE_2].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_OUTPUT_PIPE_2].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_INPUT_PIPE_3].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_OUTPUT_PIPE_3].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_INPUT_PIPE_4].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
    ...
    epInfo[XBOX_OUTPUT_PIPE_4].bmNakPower =
        USB_NAK_NOWAIT;  // Only poll once for interrupt endpoints
...
uint8_t XBOXRECV::Release() {
    ...
    for (uint8_t i = 0; i < 4; i++) Xbox360Connected[i] = 0x00;
...
uint8_t XBOXRECV::Poll() {
    if (!bPollEnable)
        return 0;
    if (!checkStatusTimer || ((int32_t)((uint32_t)millis() - checkStatusTimer) >
                              3000)) {  // Run checkStatus every 3 seconds
    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        if (bufferSize > 0) {  // The number of received bytes
            ...
            readReport(i);
#ifdef PRINTREPORT
            printReport(i, bufferSize);  // Uncomment "#define PRINTREPORT" to print the report send
                                         // by the Xbox 360 Controller
#endif
        }
    }
    return 0;
...
void XBOXRECV::readReport(uint8_t controller) {
    ...
    //     return;
    // }
    // if (readBuf[1] != 0x01)  // Check if it's the correct report - the receiver also sends
    // different
    //                          // status reports
    //     return;
    ...
    //     (uint32_t)(readBuf[9] | ((uint16_t)readBuf[8] << 8) | ((uint32_t)readBuf[7] << 16) |
    //                ((uint32_t)readBuf[6] << 24));
    ...
    //     ButtonClickState[controller] =
    //         (ButtonState[controller] >> 16) &
    //         ((~OldButtonState[controller]) >> 16);  // Update click state variable, but don't
    //                                                 // include the two trigger buttons L2 and R2
    //     if (((uint8_t)OldButtonState[controller]) == 0 &&
    //         ((uint8_t)ButtonState[controller]) !=
    //             0)  // The L2 and R2 buttons are special as they are analog buttons
    //         R2Clicked[controller] = true;
    //     if ((uint8_t)(OldButtonState[controller] >> 8) == 0 &&
    //         (uint8_t)(ButtonState[controller] >> 8) != 0)
    //         L2Clicked[controller] = true;
    //     OldButtonState[controller] = ButtonState[controller];
    // }
}

void XBOXRECV::printReport(uint8_t controller __attribute__((unused)), uint8_t nBytes
                           __attribute__((unused))) {  // Uncomment "#define PRINTREPORT" to print
                                                       // the report send by the Xbox 360 Controller
#ifdef PRINTREPORT
    if (readBuf == NULL)
        return;
    Notify(PSTR("Controller "), 0x80);
...
ControllerStatus[controller] & 0x0008   // headset adapter plugged in, but no headphones connected
(mute?) ControllerStatus[controller] & 0x0010   // 0 ControllerStatus[controller] & 0x0020   // 1
...
void XBOXRECV::setLedMode(LEDModeEnum ledMode,
                          uint8_t     controller) {  // This function is used to do some speciel LED
...
void XBOXRECV::updateGuitarState() {
    if (pkt.command != 0x14)
        return;
