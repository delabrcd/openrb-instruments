...
USBH_MIDI::USBH_MIDI(USB *p)
    : pUsb(p), bAddress(0), bPollEnable(false), readPtr(0), UsbMidiConnected(false) {
...
uint8_t USBH_MIDI::Init(uint8_t parent, uint8_t port, bool lowspeed) {
    ...
    Notify(PSTR("USBH_MIDI version "), 0x80),
        D_PrintHex((uint8_t)(USBH_MIDI_VERSION / 10000), 0x80),
        D_PrintHex((uint8_t)(USBH_MIDI_VERSION / 100 % 100), 0x80),
        D_PrintHex((uint8_t)(USBH_MIDI_VERSION % 100), 0x80), Notify(PSTR("\r\n"), 0x80);
#endif
    ...
    if (bNumEP == 1) {  // Device not found.
        ...
        for (uint8_t i = 0; i < num_of_conf; i++) {
            MidiDescParser midiDescParser(
                this, false);  // Allow all devices, vendor specific class with Bulk transfer
            rcode = pUsb->getConfDescr(bAddress, 0, i, &midiDescParser);
            if (rcode)  // Check error code
                goto FailGetConfDescr;
            bNumEP += midiDescParser.getNumEPs();
...
void USBH_MIDI::setupDeviceSpecific() {
    ...
    if (vid == 0x1235) {
        ...
        // LaunchKey: 0x30-32,  0x35:Mini, 0x7B-0x7D:MK2, 0x0102,0x113-0x122:MiniMk3,
        // 0x134-0x137:MK3
...
uint8_t USBH_MIDI::SendData(uint8_t *dataptr, uint8_t nCable) {
    ...
    Notify(PSTR("SendData():"), 0x80), D_PrintHex((buf[0]), 0x80), D_PrintHex((buf[1]), 0x80),
        D_PrintHex((buf[2]), 0x80), D_PrintHex((buf[3]), 0x80), Notify(PSTR("\r\n"), 0x80);
#endif
...
void MidiDescParser::Parse(const uint16_t len, const uint8_t *pbuf,
            ...
            switch (dscrType) {
                ...
                case USB_DESCRIPTOR_INTERFACE:
                    ...
                    if (isMidiSearch) {  // true: MIDI Streaming, false: ALL
                        if (uid->bInterfaceClass == USB_CLASS_AUDIO &&
                ...
                case USB_DESCRIPTOR_ENDPOINT:
                    ...
                    if (isGoodInterface && nEPs < 2) {
                        ...
                        if (theXtractor->EndpointXtract(confValue, 0, 0, 0,
